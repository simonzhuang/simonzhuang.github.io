...
def add_trees(t1, t2):
    ...
    if t1.is_leaf() or t2.is_leaf():
        return Tree(t1.root + t2.root, t1.branches + t2.branches)
    else:
        manualzip = []
        for k in range (0, max(len(t1.branches), len(t2.branches))):
            if k < len(t1.branches) and k < len(t2.branches):
                manualzip += [(t1.branches[k], t2.branches[k])]
            elif k >= len(t1.branches) and k < len(t2.branches):
                manualzip += [(Tree(0), t2.branches[k])]
            elif k < len(t1.branches) and k >= len(t2.branches):
                manualzip += [(t1.branches[k],  Tree(0))]
        return Tree(t1.root + t2.root, [add_trees(a, b) for a, b in manualzip])
