class Fib():
    ...
    def next(self):
        if self.value == 0:
            self.previous = 1
        next_fib_obj = Fib()
        next_fib_obj.previous, next_fib_obj.value = self.value, self.previous + self.value
        return next_fib_obj
...
class MissManners:
    ...
    def ask(self, message, *args):
        magic_word = 'please '
        rest = message[len(magic_word)::]
        if not message.startswith(magic_word):
            return 'You must learn to say please first.'
        if not hasattr(self.obj, rest):
            return 'Thanks for asking, but I know not how to {}.'.format(rest)
